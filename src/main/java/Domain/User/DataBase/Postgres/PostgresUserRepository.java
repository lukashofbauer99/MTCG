...
public class PostgresUserRepository implements IUserRepository {
    ...
    public User findEntity(Long id) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                //region battles
                PreparedStatement statementBattle = _connection.prepareStatement("""
                        SELECT 
                        id
                        FROM battles
                        where user1=?
                ...
                statementBattle.setLong(1, id);
                ResultSet resultSetBattle = statementBattle.executeQuery();
                while (resultSetBattle.next()) {
                    user.getBattleHistory().add(findBattle(resultSetBattle.getLong(1)));
                }
                statementBattle = _connection.prepareStatement("""
                        SELECT 
                        id
                        FROM battles
                        where user2=?
                ...
                statementBattle.setLong(1, user.getId());
                resultSetBattle = statementBattle.executeQuery();
                while (resultSetBattle.next()) {
                    user.getBattleHistory().add(findBattle(resultSetBattle.getLong(1)));
                }
                //endregion
    ...
    public Battle findBattle(Long id) {
        ...
        try {
            ...
            while (resultSet.next()) {
                battle = new Battle(
                        resultSet.getLong(1),
                        findUserWithoutBattles(resultSet.getLong(2)),
                        findUserWithoutBattles(resultSet.getLong(3)),
                        findUserWithoutBattles(resultSet.getLong(4))
                );
                //endregion
                //region rounds
                PreparedStatement statementStack = _connection.prepareStatement("""
    ...
    public User findUserWithoutBattles(Long id) {
        User user = null;
        try {
            //region user data
            PreparedStatement statement = _connection.prepareStatement("""
